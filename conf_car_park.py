...
XC1 = -10                                                                                                   # X coord center ellipse 1
YC1 = 6.75                                                                                                  # Y coord center ellipse 1
A1  = 17                                                                                                    # Width ellipse 1 
B1  = 4.5                                                                                                   # Height ellipse 1 
XC2 = 10                                                                                                    # X coord center ellipse 2 
YC2 = 6.75                                                                                                  # Y coord center ellipse 2
A2  = 17                                                                                                    # Width ellipse 2 
B2  = 4.5                                                                                                   # Height ellipse 2 
XC3 = 0                                                                                                     # X coord center ellipse 2 
YC3 = -2                                                                                                    # Y coord center ellipse 2
A3  = 40                                                                                                    # Width ellipse 2 
...
check_points_BF = np.array([[-L/2,        W/2], 
                            [-L/2+L/3,    W/2],
                            [-L/2+2/3*L,  W/2],
                            [L/2,         W/2],
                            [L/2,           0],
                            ...
                            [-L/2+L/3,   -W/2],
                            [-L/2,       -W/2],
                            [-L/2,         0]])  # Check points in BF                                                                     # Check points in BF

### Weigths
w_d = 1e2                                                                                                   # Distance from target weight
w_u = 1e1                                                                                                   # Control effort weight
w_peak = 1e6                                                                                                # Target threshold weight
w_ob = 5e4                                                                                                  # Obstacle weight
w_v = 1e2                                                                                                   # Velocity weight
delta_bound = 2*np.pi/6
w_delta_bound = 0                                                                                           # Delta bound weight (not implemented)
weight = np.array([w_d, w_u, w_peak, w_ob, w_v])                                                            # Weights vector (tmp)
cost_weights_running  = np.array([w_d, w_peak, 0., w_ob, w_ob, w_ob, w_u, w_delta_bound])                   # Running cost weights vector
cost_weights_terminal = np.array([w_d, w_peak, w_v, w_ob, w_ob, w_ob, 0, w_delta_bound])                    # Terminal cost weights vector 

k_db = 50

### SoftMax parameters 
alpha = 50                                                                                                  # Soft abs coefficient (obstacle) 
alpha2 = 1                                                                                                  # Soft abs coefficient (peak)
...
### Target parameters
x_des = 0                                                                                                   # Target x position
y_des = 6.75                                                                                                # Target y position
...
Config_path = './Results Car Park/Results {}/Configs/'.format(test_set)                                     # Configuration path
Fig_path = './Results Car Park/Results {}/Figures'.format(test_set)                                         # Figure path
NNs_path = './Results Car Park/Results {}/NNs'.format(test_set)                                             # NNs path
Log_path = './Results Car Park/Results {}/Log/'.format(test_set)                                            # Log path
Code_path = './Results Car Park/Results {}/Code/'.format(test_set)                                          # Code path
DictWS_path = './Results Car Park/Results {}/DictWS/'.format(test_set)                                      # DictWS path
path_list = [Fig_path, NNs_path, Log_path, Code_path, DictWS_path]                                          # Path list

# Recover-training parameters
test_set_rec = None
NNs_path_rec = './Results Car Park/Results set {}/NNs'.format(test_set_rec)                                 # NNs path recover training
N_try_rec = None
...
x_init_min = np.array([-10, 1.5, -math.pi/6, 0, 0, 0]) #1.5 #-2.5                                           # State lower bound initial configuration array
x_max = np.array([np.inf, np.inf, np.inf, np.inf, np.inf, np.inf])                                          # State upper bound vector
x_init_max = np.array([ 10, 3, math.pi/6, 0, 0, (NSTEPS-1)*dt]) #3.0 #10                                    # State upper bound initial configuration array
state_norm_arr = np.array([10, 3, math.pi, 10, math.pi/6, int(NSTEPS*dt)])                                  # Array used to normalize states
# state: x, y, theta, v, delta, t

# initial configurations for plot.rollout()
init_states_sim = [np.array([-9.0 - L_delta, 2.0, 0.0, 0.0, 0.0, 0.0]),
                   np.array([-5.0 - L_delta, 2.0, 0.0, 0.0, 0.0, 0.0]),
                   np.array([-2.5 - L_delta, 2.0, 0.0, 0.0, 0.0, 0.0]),
                   np.array([0.0  - L_delta, 2.0, 0.0, 0.0, 0.0, 0.0]),
                   np.array([2.5  - L_delta, 2.0, 0.0, 0.0, 0.0, 0.0]),
                   np.array([5.0  - L_delta, 2.0, 0.0, 0.0, 0.0, 0.0]),
                   np.array([9.0  - L_delta, 2.0, 0.0, 0.0, 0.0, 0.0])]

### Action parameters
nb_action = 2                                                                                               # Action size
acc_lower_bound = -3                                                                                        # Action lower bound
acc_upper_bound = 3     
delta_dot_lower_bound = -1
...
### Plot parameters
fig_ax_lim = np.array([[-11, 11], [-2.5, 10]])                                                              # Figure axis limit [x_min, x_max, y_min, y_max]
