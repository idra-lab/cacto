...
from utils import normalize_tensor, custom_logarithm
...
class NN:
    ...
    def create_actor(self):
        ''' Create actor NN '''
        if (self.conf.system_id == 'car' or self.conf.system_id == 'car_park' or self.conf.system_id == 'bicopter') and self.conf.remap_angle:
        ...
        elif self.conf.system_id == 'bicopter' and self.conf.remap_angle:
            nb_state_input = self.conf.nb_state+1
        else:
        ...
        lay1 = layers.Dense(self.conf.NH1,kernel_regularizer=regularizers.l1_l2(self.conf.kreg_l1_A,self.conf.kreg_l2_A),bias_regularizer=regularizers.l1_l2(self.conf.breg_l1_A,self.conf.breg_l2_A))(state_input)                                        
        ...
        if self.conf.system_id == 'bicopter':
            outputs = (outputs+1)/2*self.conf.u_max
        else:
            outputs = outputs*self.conf.u_max
    ...
    def create_critic_sine(self): 
        ...
        state_out1 = SinusodialRepresentationDense(64, activation='sine', kernel_regularizer=regularizers.l2(self.conf.kreg_l2_C),bias_regularizer=regularizers.l2(self.conf.breg_l2_C))(state_input) 
        state_out2 = SinusodialRepresentationDense(64, activation='sine', kernel_regularizer=regularizers.l2(self.conf.kreg_l2_C),bias_regularizer=regularizers.l2(self.conf.breg_l2_C))(state_out1) 
        out_lay1 = SinusodialRepresentationDense(128, activation='sine', kernel_regularizer=regularizers.l2(self.conf.kreg_l2_C),bias_regularizer=regularizers.l2(self.conf.breg_l2_C))(state_out2)
        out_lay2 = SinusodialRepresentationDense(128, activation='sine', kernel_regularizer=regularizers.l2(self.conf.kreg_l2_C),bias_regularizer=regularizers.l2(self.conf.breg_l2_C))(out_lay1)
    ...
    def create_std_critic_sine(self): 
        ''' Create critic NN - elu'''
        if (self.conf.system_id == 'car' or self.conf.system_id == 'car_park' or self.conf.system_id == 'bicopter') and self.conf.remap_angle:
            nb_state_input = self.conf.nb_state+1
        elif self.conf.system_id == 'manipulator' and self.conf.remap_angle:
        ...
        state_out1 = SinusodialRepresentationDense(64, activation='sine', kernel_regularizer=regularizers.l2(0.1),bias_regularizer=regularizers.l2(0.1))(state_input) 
        state_out2 = SinusodialRepresentationDense(64, activation='sine', kernel_regularizer=regularizers.l2(0.1),bias_regularizer=regularizers.l2(0.1))(state_out1) 
        out_lay1 = SinusodialRepresentationDense(128, activation='sine', kernel_regularizer=regularizers.l2(0.1),bias_regularizer=regularizers.l2(0.1))(state_out2)
        out_lay2 = SinusodialRepresentationDense(128, activation='sine', kernel_regularizer=regularizers.l2(0.1),bias_regularizer=regularizers.l2(0.1))(out_lay1)
    ...
    def eval(self, NN, input):
        ...
        if (self.conf.system_id == 'car' or self.conf.system_id == 'car_park') and self.conf.remap_angle:
            ...
            input = tf.stack([input[:,0],input[:,1],tf.cos(input[:,2]),tf.sin(input[:,2]),input[:,3],input[:,4],input[:,5],input[:,6]], axis=1)
    ...
    def compute_critic_grad(self, critic_model, target_critic, state_batch, state_next_rollout_batch, partial_reward_to_go_batch, dVdx_batch, d_batch, weights_batch):
        ...
        with tf.GradientTape() as tape: 
            ...
            if self.w_S != 0:
                ...
                critic_loss_der = self.MSE(custom_logarithm(dVdx_batch), custom_logarithm(der_critic_value[:,:self.conf.nb_state-1]), sample_weight=weights_batch) # dV/dt not computed and so not used in the update
        ...
        return critic_grad, critic_loss, critic_loss, self.eval(target_critic, state_batch)
    
    def compute_std_citic_grad(self, std_critic_model, critic_model, target_critic, state_batch, state_next_rollout_batch, partial_reward_to_go_batch, dVdx_batch, d_batch, weights_batch):
        # Compute value function tail if TD(n) is used
        if self.conf.MC:
            reward_to_go_batch = partial_reward_to_go_batch
        else:     
        ...
        critic_value = self.eval(critic_model, state_batch)   
        critic_error = reward_to_go_batch-critic_value
        
        with tf.GradientTape() as tape: 
            # Compute critic loss
            log_std_critic_value = self.eval(std_critic_model, state_batch)
            exp_std_critic_value = tf.exp(log_std_critic_value)
            std_critic_loss = tf.reduce_mean(0.5*log_std_critic_value + 0.5*tf.square(critic_error)/exp_std_critic_value)

        # Compute the gradients of the critic loss w.r.t. critic's parameters
        std_critic_grad = tape.gradient(std_critic_loss, std_critic_model.trainable_variables)   

        return std_critic_grad
    ...
    def compute_actor_grad(self, actor_model, critic_model, state_batch, term_batch, batch_size):
        ...
        state_next_tf, ds_next_da = self.env.simulate_batch(state_batch.numpy(), actions.numpy()) , self.env.derivative_batch(state_batch.numpy(), actions.numpy())# * self.conf.dt / dt
        ...
        weights = term_batch.dot(cost_weights_terminal_reshaped) + (1-term_batch).dot(cost_weights_running_reshaped)
        with tf.GradientTape() as tape1:
            tape1.watch(actions)
            rewards_tf = self.env.reward_batch(weights, state_batch.numpy(), actions)
